...
export class SearchQueryHandler implements IQueryHandler<SearchQuery, SearchResultDto[]> {
  ...
  async execute(query: SearchQuery): Promise<SearchResultDto[]> {
    this.logger.debug({ q: query.q }, 'Executing runbook search query');
    const runbooks = await this.searchService.search(query.q);
    return plainToInstance(SearchResultDto, runbooks, { excludeExtraneousValues: true });
  }
}
